...
class Pictures extends Component {
  ...
  getPhotos = (start, album) => {
    axios
      ...
      .catch(() => {
        ...
        if(modalShow){
          this.setState(prevState => ({
            modalPhoto: prevState.modalPhoto+1,
          }));
        }
      });
  };
  ...
  clickR = () => {
    let index = this.state.modalPhoto;
    const { albumLength, albumTitle, images } = this.state;
    ...
    if (index === currLength && albumLength - 1 > currLength) {
      this.getPhotos(images.length, albumTitle.toLowerCase())
    }
  };
  ...
  render() {
    ...
    // const isInstaVisible = images.length === albumLength;
    if (redirect) {
