...
class Entries extends Component {
  ...
  submitHandler = (e) => {
    ...
    if (myArticle.title && myArticle.date && myArticle.text) {
      if (
        this.state.updateArticleKey &&
        this.state.entries[this.state.updateArticleKey].date === this.state.date
      ) {
        write
          .patch(
            `${this.props.userId}/${this.state.updateArticleKey}/.json?auth=${this.props.idToken}`,
            myArticle
          )
          .then((response) => {
        ...
        write
          .post(
            `${this.props.userId}.json?auth=${this.props.idToken}`,
            myArticle
          )
          .then((response) => {
            this.props.Header("Post Successful");
  ...
  deleteHandler = (articleToDelete) => {
    write
      .delete(
        `${this.props.userId}/${articleToDelete}/.json?auth=${this.props.idToken}`
      )
      .then((response) => {
    ...
    return (
      <div>
        ...
        <div className={entriesCSS.buttonPosition}>
          <button
            className={entriesCSS.publishButtons}
