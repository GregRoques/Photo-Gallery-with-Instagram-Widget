...
const projectDetails = {
  ...
  pizzaCat: {
    ...
    readMe:
      "https://github.com/GregRoques/Pizza-Cat-Python-Project/blob/master/README.md",
  },
};
...
const ProjectList = ({ name, click, hover, offHover }) => {
  return (
    <div>
      <div
        className={`optionCircles ${name}BackgroundImage`}
        onClick={() => click(name)}
        onMouseOut={() => offHover()}
        onMouseOver={() => hover(name)}
      ></div>
    </div>
  );
};
  ...
  return (
    <div className="grayColumnContent">
      <div className="nonVidPicColumn">
        <img
          className="picPortfolio p1"
          src={headerImageLink + display["image"] + ["1.png"]}
          alt={display["name"]}
        />
        <img
          className="picPortfolio p2"
          src={headerImageLink + display["image"] + ["2.png"]}
          alt={display["name"]}
        />
      </div>
      <div className="nonVidTextColumn">
        ...
        <div className="buttonAlign v1">
          <span>
            <a target="_blank" rel="noopener noreferrer" href={display["demo"]}>
              <button className="demoReadButtons">
                {display["demo"].includes("youtube", -1)
                  ? `${"Video"}`
                  : `${"Live Demo"}`}
              </button>
            </a>
      ...
      {display["languages"].map((language, i) => {
        ...
        return (
          <img
            className="devStyle"
            key={i}
            src={languageImageLink + language}
            alt={altText}
          />
        );
      })}
  ...
  render() {
    return (
      <div className="portfolioStyling fadeIn">
        ...
        <div className="hoverPageLine h1">{this.state.hoverProject}</div>
        <div className="parameters">
          {Object.keys(projectDetails).map((project, i) => {
            return this.state.currentProject === project ? (
